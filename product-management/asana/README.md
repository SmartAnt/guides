Product management process with Asana
======================================


Our product management process with [Asana](http://asana.com) can span multiple
Trello boards:
...
Items from Product Design, Bugs, Engineering boards are pulled into the Next Up
...
Items start out as a simple idea, 1-2 sentences long. As they are pulled through
projects, detail is added, explaining why (from a business perspective)
...
The items on this project are bugs in the described state. There are multiple
lists on this board:
...
The items on this board lists are the result of sketching user flows, usability
...
The items on this board are refactorings and other engineering tasks necessary
...
* Re-sort the entire Next Up queue according to priority. Items that were at the
...
* Review the Engineering & Refactoring board. Pull items into Next Up based on
...
* Review the items that went live last week, taking 30 seconds to describe each
  card, and congratulate those who worked on them, showing their avatar.
* Review the items in Next Up. Clarify any questions about them.
...
The items in the Next Up list are prioritized, vetted, and ready for design &
...
can handle, but should consider the items are prioritized by importance.

The items in the In Progress list are actively being designed or developed.
Etiquette is that you should never have your face on more than two items at a
...
The items in the Testing on Staging (or Testing on Ad Hoc build for iPhone apps)
...
The items in the Ready for Production list include items that have been accepted
...
The items in the Live (Week of [date]) lists have been released. Each week has
